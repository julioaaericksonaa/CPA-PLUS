...
import { compareUpstreamVersions, readApiLatestVersion, readManagerLatestTag, type UpstreamVersionState } from '@/features/system/versionChecks';
...
const renderBadge = (
  ...
  if (state === 'update') {
    return {
      label: t('dashboard.version_update_available', { version: latest.trim() }),
...
export function VersionCard({
  ...
  const handleAppVersionCheck = useCallback(async () => {
    ...
    try {
      ...
      const comparison = compareUpstreamVersions(latestApp, appVersion);
  ...
  const handleApiVersionCheck = useCallback(async () => {
    ...
    try {
      ...
      const comparison = compareUpstreamVersions(latestApi, apiVersion);
