...
CPA-PLUS integrated builds use the single-port entry `http://host:8317/management.html`, one `remote-management.secret-key`, and persistent Plus data at `./data/usage.sqlite` by default. The integrated build now includes the bundled Plus panel, SQLite-backed request monitoring, usage import/export, API-key aliases, dashboard analytics, model prices with LiteLLM sync, and the in-process usage collector. See [CPA-PLUS integrated deployment](docs/cpa-plus-integrated.md).
